...
module Launchpad {
    use 0x1::Account;
    use 0x1::LibraCoin; // Replace with your custom token if needed
    use 0x1::Event;
    ...
    public fun create_sale(
        ...
        let current_time = LibraTimestamp::now_seconds();
    ...
    public fun purchase_tokens(
        ...
        assert!(LibraTimestamp::now_seconds() <= sale_ref.sale_end, 1001);
        ...
        LibraCoin::burn(investor, cost);
        LibraCoin::deposit(&sale_ref.creator, cost);
    ...
    public fun apply_vesting_schedule(investor: &signer, amount: u64, vesting_schedule: &vector<u64>) {
        // Implement vesting logic here. For simplicity, this example does not provide actual vesting.
    }
    ...
    public fun close_sale(sales_manager_addr: address, sale_id: u64, creator_account: &signer) {
        ...
        LibraCoin::deposit(creator_account, remaining_tokens);
    }
}